...
async function fetchPosts(page = 1) {
    try {
        ...
        const res = await axios.get("/posts/data", { params });
...
async function submit() {
    ...
    try {
        if (editingId.value) {
            // include CSRF token as fallback
...
async function removePost(post) {
    ...
    try {
        // send CSRF token in request body for delete as axios.delete doesn't send body by default
